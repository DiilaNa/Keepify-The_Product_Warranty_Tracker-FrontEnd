  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between gap-2 mt-4">
        <Button
          onClick={() => window.open(billImage, "_blank")}
          className="flex-1 rounded-lg text-xs font-medium bg-blue-600 hover:bg-blue-700 text-white"
        >
          Bill
        </Button>
        ...
        <Button
          onClick={() => onDelete(id)}
          className="flex-1 rounded-lg text-xs font-medium bg-red-600 hover:bg-red-700 text-white"
        >
          Delete
        </Button>
      </div>
      ...
      <AdminPopup
        ...
        onSubmit={async (form) => {
          ...
          const success = await onEdit(form, id);
          if (success) closeButtonRef.current?.click();
        }}
        closeButtonRef={closeButtonRef}
        loading={loadingWarranties}
        fields={[]}
